    ...
    public class WatchShift : Entity
    {
        ...
        public class Validator : AbstractValidator<WatchShift>
        {
            ...
            public Validator()
            {
                ...
                RuleFor(x => x.ShiftType).NotNull().Must((shift, type) => type.Command == shift.Watchbill.Command)
                    .WithMessage("A watch shift's type must belong to the same command as the watchbill.");
