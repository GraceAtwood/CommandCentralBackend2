...
namespace CommandCentral.Utilities
{
    ...
    public static class AuthorizationUtilities
    {
        ...
        /// <returns></returns>
        public static List<PermissionGroup> GetPermissionGroupsFromNames(IEnumerable<string> groupNames, bool includeDefaults)
        {
            List<PermissionGroup> groups = new List<PermissionGroup>();

            foreach (var groupName in groupNames)
            {
                var group = PermissionGroup.AllPermissionGroups.FirstOrDefault(x => x.GroupName.SafeEquals(groupName));

                if (group == null)
                    throw new Exception($"The group name, '{groupName}', was not valid.");
        ...
        /// <returns></returns>
        public static bool CanAccessSubmodules(this IEnumerable<PermissionGroup> groups, params string[] submodules)
        {
        ...
        public static ResolvedPermissions ResolvePermissions(this Entities.Person client, Entities.Person person)
        {
            ...
            foreach (var group in groups)
            {
                ...
                foreach (var coc in group.ChainsOfCommandParts)
                {
                    ...
                    coc.PropertyGroups
                        .Where(x => x.AccessCategory == AccessCategories.Return && (!x.Disjunctions.Any() || x.Disjunctions.All(y => y.Rules.All(z => z is Authorization.Rules.IfInChainOfCommandRule))))
                        .SelectMany(x => x.Properties)
                        .ToList()
                        .ForEach(x =>
                        {
