...
namespace CommandCentral.ChangeEvents
{
    /// <summary>
        ...
        public static IEnumerable<System.Net.Mail.MailAddress> GetValidSubscriptionEmailAddresses(Person person, IChangeEvent changeEvent)
        {
            ...
            using (var session = DataProvider.CurrentSession)
            {
                ...
                if (changeEvent.RestrictToChainOfCommand)
                {
                    ...
                    var personIds = chainOfCommand[ChainsOfCommand.Main].SelectMany(x => x.Value.Select(y => y.Id)).ToList();
                ...
                if (person != null)
                {
                    ...
                    subscribers = subscribers.Where(subscriber =>
                    {
                        ...
                        throw new Exception($"While processing the change event, '{changeEvent.Id}', we found a subscription to that event with an invalid level: '{subscriptionEvent.Value}'.");
                    }).ToList();
                }
